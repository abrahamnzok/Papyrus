...
/**
 * Object of type Receiver Handles all the actions executed by the concrete commands {@link
 * classes.concretecommands.Insert}, {@link classes.concretecommands.Selection}, {@link
 * classes.concretecommands.Paste}, {@link classes.concretecommands.Delete}, {@link
 * classes.concretecommands.Cut}, {@link classes.concretecommands.Copy}
 */
public class BoardReceiver implements Receiver, Recordable {

  /**
   * Object that stores the user's input When there is an insert, paste or delete action
   */
  private Buffer buffer;

  /**
   * Object that stores temporary content of the buffer When there is a copy or cut action
   */
  private ClipBoard clipboard;

  /**
   * Object that deals with the selection When there is a select action
   */
  ...
  /**
   * Preferred constructor of the engine Internal objects include {@link Buffer} {@link ClipBoard}
   * {@link Ranger}
   */
   ...
   * @param position the position where to insert the text {@param position} can never be out of
   * boundaries
   */
  @Override
   ...
   * @param end which is the ending point of the selection if the selection is set out of boundaries
   * then selection is made on the whole content
   */
  @Override
  public void select(int start, int end) {
    ...
    this.ranger.setSelection(
        this.buffer.getText().substring(this.ranger.getSpaceBegin(), this.ranger.getSpaceEnd()));
    try {
      ((DoUndoEngine) this.recorder).record(this.save());
    } catch (NoSuchMethodException | CloneNotSupportedException e) {
      e.printStackTrace();
  ...
  /**
   * handles the action of copying the selection to the clipboard. We can only perform this action
   * on the assurance that the ranger is not empty
   */
  @Override
  public void copy() {
  ...
  /**
   * handles the action of cutting the selection from the buffer and copying it to the clipboard We
   * can only perform a cut action on the assurance that the ranger is not empty
   */
  @Override
  public void cut() {
  ...
  /**
   * @param position position where the content of the clipboard is to be put in the buffer position
   * can never be set out of boundaries
   */
  @Override
  ...
  /**
   * @param position position of the character to delete position can never be set out of
   * boundaries
   */
  @Override
  ...
  /**
   * @return a Buffer's clone. A clone of object Buffer is rendered to avoid the real object being
   * modified
   */
  @Override
  ...
  /**
   * @return a Ranger's clone A clone of the object is rendered to avoid the real object being
   * modified
   */
  @Override
  ...
  /**
   * @return a Clipboard's clone A clone of the object is rendered to avoid the real object being
   * modified
   */
  @Override
